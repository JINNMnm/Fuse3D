...
def render_video(sample, resolution=1024, bg_color=(0, 0, 0), num_frames=300, r=2, fov=40, **kwargs):
...
def render_view(sample, resolution=1024, bg_color=(0, 0, 0), yaw=np.pi/2, pitch= 20 / 180 * np.pi, r=10, fov=8, **kwargs):
    extrinsic, intrinsics = yaw_pitch_r_fov_to_extrinsics_intrinsics([yaw], [pitch], r, fov)
    return render_frames(sample, extrinsic, intrinsics, {'resolution': resolution, 'bg_color': bg_color}, **kwargs)